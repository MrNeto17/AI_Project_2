(() => {
  ...
  function renderList(el, items) {
    ...
    if (el === columns.faturas) {
      // Render invoices newest-first: iterate from end to start so new entries appear on top
      for (let i = items.length - 1; i >= 0; i--) {
        const item = items[i];
        const node = _formatInvoice(item);
        el.appendChild(node);
      }
    } else {
      items.forEach((item) => {
        let node = null;
        if (el === columns.orders_queue || el === columns.orders_answered) {
  ...
  function render(newState) {
    ...
    if (pauseBtn) {
      ...
      // emoji-only label, keep green background consistent
      pauseBtn.textContent = paused ? "▶" : "⏸";
      pauseBtn.className = "btn btn-success btn-sm mb-2";
      // hide button when simulation completes
    ...
    if (state.is_complete) {
      ...
      // keep viewSelect enabled so the user can switch views after completion
    }
  }
  ...
  async function fetchState() {
    ...
    const res = await fetch(
      `/api/simulation/state?view=${encodeURIComponent(view)}`,
    );
